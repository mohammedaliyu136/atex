...
class DocumentController extends Controller
{
    public function index()
    {
        ...
        if ($user->hasRole('super-admin') || $user->hasRole('field-officer')) {
            ...
            return view('exporter.documents.admin', compact('documents'));
        }

        if ($user->hasRole('exporter')) {
            $profile = ExporterProfile::where('user_id', $user->id)->first();
            $documents = Document::where('owner_type', 'exporter')
                ->where('owner_id', $profile->id ?? 0)
                ->with('reviewer')
                ->latest()
                ->get();
            return view('exporter.documents.exporter', compact('documents', 'profile'));
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($user->hasRole('super-admin') || $user->hasRole('field-officer')) {
            $ownerType = $request->owner_type ?: 'exporter';
