...
class ProductController extends Controller
{
    public function index()
    {
        ...
        if ($user->hasRole('super-admin') || $user->hasRole('field-officer')) {
            $products = Product::with(['category', 'exporterProfile'])->latest()->get();
            $categories = Category::where('status', 'active')->get();
            return view('exporter.products.admin', compact('products', 'categories'));
        }

        if ($user->hasRole('exporter')) {
            $profile = ExporterProfile::where('user_id', $user->id)->first();
            $products = Product::where('exporter_profile_id', $profile->id ?? 0)->with('category')->latest()->get();
            $categories = Category::where('status', 'active')->get();
            return view('exporter.products.exporter', compact('products', 'categories', 'profile'));
        }
    ...
    public function store(Request $request)
    {
        ...
        $profile = ExporterProfile::where('user_id', $user->id)->first();
        if (!$profile) {
            return redirect()->back()->with('error', 'Exporter profile not found.');
        }
