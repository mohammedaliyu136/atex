...
<div class="mb-8 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Edit Buyer</h1>
        <p class="text-slate-500 text-sm">Update profile details and roles for {{ $buyer->name }}.</p>
    </div>
...
<div class="max-w-2xl">
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
        <form action="{{ route('admin.buyers.update', $buyer->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $buyer->name) }}" required
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $buyer->email) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="1"
                            ...
                            placeholder="Enter physical address">{{ old('address', $buyer->address) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <label class="flex items-center p-4 rounded-xl border border-slate-100 bg-white cursor-pointer hover:border-primary-200 transition-all shadow-sm">
                            <input type="checkbox" name="roles[]" value="{{ $role->name }}" 
                                {{ $buyer->hasRole($role->name) ? 'checked' : '' }}
