...
<div class="mb-8 flex justify-between items-center">
    <div>
        ...
        <p class="text-slate-500 text-sm">Detailed information for {{ $buyer->name }}</p>
    </div>
    <div class="flex gap-3">
        <a href="{{ route('admin.buyers.index') }}" class="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-medium flex items-center hover:bg-slate-50 transition-colors">
            <i data-lucide="arrow-left" class="w-5 h-5 mr-2"></i>
            Back to List
        </a>
        <a href="{{ route('admin.buyers.edit', $buyer->id) }}" class="px-4 py-2 bg-primary-600 text-white rounded-xl font-medium flex items-center hover:bg-primary-700 transition-colors">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 text-center">
            ...
            <h2 class="text-xl font-bold text-slate-800 mb-1">{{ $buyer->name }}</h2>
            <p class="text-slate-400 text-sm mb-4">{{ $buyer->email }}</p>
            ...
            <div class="space-y-4">
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-700 font-medium">{{ $buyer->phone ?? 'Not provided' }}</p>
                    </div>
                </div>
                <div class="flex items-start">
                    <div class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center mr-3 shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-700 font-medium">{{ $buyer->address ?? 'Not provided' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($buyer->getAllPermissions() as $permission)
            ...
            <div class="space-y-6 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-px before:bg-slate-100">
                <div class="relative pl-10">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $buyer->created_at->format('M d, Y @ H:i') }}</p>
                    </div>
                </div>
                @if($buyer->email_verified_at)
                <div class="relative pl-10">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $buyer->email_verified_at->format('M d, Y @ H:i') }}</p>
                    </div>
                </div>
                @endif
                <div class="relative pl-10">
