...
<div class="mb-8 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Edit Seller</h1>
        <p class="text-slate-500 text-sm">Update profile details and roles for {{ $seller->name }}.</p>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="1"
                            ...
                            placeholder="Enter physical address">{{ old('address', $seller->address) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <label class="flex items-center p-4 rounded-xl border border-slate-100 bg-white cursor-pointer hover:border-primary-200 transition-all shadow-sm">
                            <input type="checkbox" name="roles[]" value="{{ $role->name }}" 
                                {{ $seller->hasRole($role->name) ? 'checked' : '' }}
