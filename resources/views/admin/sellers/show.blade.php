...
<div class="mb-8 flex justify-between items-center">
    <div>
        ...
        <p class="text-slate-500 text-sm">Detailed information for {{ $seller->name }}</p>
    </div>
    <div class="flex gap-3">
        ...
        @if(!$seller->email_verified_at)
            <form action="{{ route('admin.sellers.verify-email', $seller->id) }}" method="POST" class="inline">
                @csrf
        ...
        <a href="{{ route('admin.sellers.edit', $seller->id) }}" class="px-4 py-2 bg-primary-600 text-white rounded-xl font-medium flex items-center hover:bg-primary-700 transition-colors">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 text-center">
            ...
            <h2 class="text-xl font-bold text-slate-800 mb-1">{{ $seller->name }}</h2>
            <p class="text-slate-400 text-sm mb-4">{{ $seller->email }}</p>
            ...
            <div class="space-y-4">
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-700 font-medium">{{ $seller->phone ?? 'Not provided' }}</p>
                    </div>
                </div>
                <div class="flex items-start">
                    <div class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center mr-3 shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-700 font-medium">{{ $seller->address ?? 'Not provided' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($seller->getAllPermissions() as $permission)
            ...
            <div class="space-y-6 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-px before:bg-slate-100">
                <div class="relative pl-10">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $seller->created_at->format('M d, Y @ H:i') }}</p>
                    </div>
                </div>
                @if($seller->email_verified_at)
                <div class="relative pl-10">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $seller->email_verified_at->format('M d, Y @ H:i') }}</p>
                    </div>
                </div>
                @endif
                <div class="relative pl-10">
