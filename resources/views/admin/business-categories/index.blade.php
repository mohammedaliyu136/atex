...
@section('header_title', 'Business Categories')
...
<div class="space-y-6">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h2 class="text-lg font-semibold text-slate-800">Manage Business Categories</h2>
        </div>
        
        @if(auth()->user()->hasPermissionTo('manage business category'))
        <div class="p-6 bg-slate-50 border-b border-slate-200">
            <form action="{{ route('admin.business-categories.store') }}" method="POST" class="flex gap-4 items-end max-w-lg">
                @csrf
                <div class="flex-1">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Add New Category</label>
                    <input type="text" name="name" class="form-input w-full" placeholder="e.g. Agriculture" required>
                </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <form action="{{ route('admin.business-categories.toggle-status', $category->id) }}" method="POST">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.business-categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ auth()->user()->hasPermissionTo('manage business category') ? 3 : 2 }}" class="px-6 py-8 text-center text-slate-500">
                                No categories found.
                            </td>
                        </tr>
                    @endforelse
