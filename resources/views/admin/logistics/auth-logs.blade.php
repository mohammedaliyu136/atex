...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div class="flex items-center">
        ...
        <div>
            ...
            <p class="text-slate-500 text-sm">Reviewing access logs for <span class="font-bold text-slate-700">{{ $logistics->name }}</span></p>
        </div>
    </div>
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.logistics.index') }}" class="px-4 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-sm flex items-center hover:bg-slate-50 transition-all shadow-sm">
            ...
            <div class="px-6 pb-6 -mt-12 text-center">
                <div class="inline-block p-1 bg-white rounded-3xl mb-4 shadow-sm">
                    @if($logistics->passport)
                        <img src="{{ $logistics->passport }}" class="w-24 h-24 rounded-2xl object-cover">
                    @else
                ...
                <h3 class="text-lg font-bold text-slate-800">{{ $logistics->name }}</h3>
                <p class="text-sm text-slate-400 mb-4">{{ $logistics->email }}</p>
                ...
                <div class="grid grid-cols-2 gap-3 pt-6 border-t border-slate-50">
                    ...
                    <div class="text-center">
                        ...
                        <span class="px-2 py-0.5 {{ $logistics->is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600' }} text-[10px] font-black rounded-md uppercase tracking-wider">
                            {{ $logistics->is_active ? 'Active' : 'Suspended' }}
                        </span>
                    </div>
                </div>
            </div>
