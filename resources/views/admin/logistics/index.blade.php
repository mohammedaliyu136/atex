...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Logistics Management</h1>
        <p class="text-slate-500 text-sm">Manage platform access and permissions</p>
    </div>
    <div class="flex items-center gap-3">
        @can('users logs')
        <a href="{{ route('admin.logistics.all-auth-logs') }}" class="px-4 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-sm flex items-center hover:bg-slate-50 transition-all shadow-sm">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    ...
    <a href="{{ request('view') === 'trash' ? route('admin.logistics.index') : route('admin.logistics.index', ['view' => 'trash']) }}" 
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50"
         x-data="{ 
            ...
            isAllSelected: false,
            toggleAll() {
                if (this.selectedUsers.length === {{ $logistics->count() }}) {
                    this.selectedUsers = [];
                    this.isAllSelected = false;
                } else {
                    this.selectedUsers = [ @foreach($logistics as $logistics) '{{ $logistics->id }}', @endforeach ];
         ...
         x-init="$watch('selectedUsers', value => isAllSelected = value.length === {{ $logistics->count() }})">
    ...
    <div class="p-6 border-b border-slate-50 flex flex-wrap items-center justify-between gap-4">
        ...
        <form action="{{ route('admin.logistics.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
    ...
    <form x-ref="bulkForm" action="{{ route('admin.logistics.bulk-action') }}" method="POST" style="display: none;">
        @csrf
    ...
    <div class="min-h-[450px]">
        <table class="w-full text-left border-separate border-spacing-0">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="group hover:bg-slate-50/50 transition-all" :class="selectedUsers.map(id => Number(id)).includes({{ $logistics->id }}) ? 'bg-slate-50' : ''">
                    <td class="px-8 py-5">
                        <div class="flex items-center">
                            <div class="mr-4">
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" class="hidden" value="{{ $logistics->id }}" x-model="selectedUsers">
                                    <div class="w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center"
                                         :class="selectedUsers.map(id => Number(id)).includes({{ $logistics->id }}) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-200 group-hover:border-emerald-500'">
                                        <i data-lucide="check" class="w-3 h-3 text-white" x-show="selectedUsers.map(id => Number(id)).includes({{ $logistics->id }})"></i>
                                    </div>
                                </label>
                            </div>
                            <div class="w-11 h-11 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden mr-4 group-hover:scale-105 transition-transform">
                                ...
                                @else
                                    <span class="text-xs font-black text-slate-400 uppercase">{{ substr($logistics->name, 0, 2) }}</span>
                                @endif
                            </div>
                            <div>
                                <p class="text-sm font-bold text-slate-800 leading-none mb-1.5">{{ $logistics->name }}</p>
                                <div class="flex items-center gap-2">
                                    <p class="text-xs text-slate-400">{{ $logistics->email }}</p>
                                    @if(!$logistics->email_verified_at)
                    ...
                    <td class="px-8 py-5">
                        <div class="flex justify-center">
                            @foreach($logistics->roles as $role)
                    ...
                    <td class="px-8 py-5 text-center">
                        ...
                        @elseif($logistics->locked_until && $logistics->locked_until->isFuture())
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-50 text-amber-600" title="Locked until {{ $logistics->locked_until->format('M d, H:i') }}">
                                <i data-lucide="lock" class="w-3 h-3 mr-1.5"></i>
                                Locked
                            </span>
                        @elseif($logistics->is_active)
                    ...
                    <td class="px-8 py-5 text-sm font-medium text-slate-500">
                        {{ $logistics->created_at->format('j/n/Y') }}
                    </td>
                    <td class="px-8 py-5 text-right" x-data="{ open: false }">
                        <div class="relative inline-block text-left">
                            ...
                            <div x-show="open" 
                                ...
                                @if(request('view') === 'trash')
                                @can('users delete')
                                <form action="{{ route('admin.logistics.restore', $logistics->id) }}" method="POST" class="contents">
                                    @csrf
                                ...
                                <form action="{{ route('admin.logistics.force-delete', $logistics->id) }}" method="POST" class="contents" onsubmit="return confirm('PERMANENTLY DELETE this account? This cannot be undone!')">
                                    @csrf @method('DELETE')
                                ...
                                @endcan
                                @else
                                <a href="{{ route('admin.logistics.show', $logistics->id) }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
                                    <i data-lucide="eye" class="w-4 h-4 mr-3 text-slate-400"></i>
                                    View Profile
                                </a>
                                @can('users logs')
                                <a href="{{ route('admin.logistics.auth-logs', $logistics->id) }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
                                ...
                                <form action="{{ route('admin.logistics.resend-verification', $logistics->id) }}" method="POST" class="contents">
                                    @csrf
                                ...
                                @can('users reset 2fa')
                                @if($logistics->hasTwoFactorEnabled())
                                <form action="{{ route('admin.logistics.reset-2fa', $logistics->id) }}" method="POST" class="contents" onsubmit="return confirm('Are you sure you want to reset 2FA for this user? They will be forced to set it up again on their next login.')">
                                    @csrf
                                ...
                                @can('users unlock')
                                @if($logistics->locked_until && $logistics->locked_until->isFuture())
                                <form action="{{ route('admin.logistics.unlock', $logistics->id) }}" method="POST" class="contents">
                                    @csrf
                                    <button class="w-full flex items-center px-4 py-2.5 text-sm text-amber-600 hover:bg-amber-50 transition-colors">
                                ...
                                @can('users edit')
                                <a href="{{ route('admin.logistics.edit', $logistics->id) }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
                                ...
                                @can('users email')
                                <button @click="$dispatch('open-email-modal', { id: {{ $logistics->id }}, name: '{{ $logistics->name }}' })" 
                                ...
                                @can('users status')
                                <form action="{{ route('admin.logistics.toggle-status', $logistics->id) }}" method="POST" class="contents">
                                    @csrf
                                    <button class="w-full flex items-center px-4 py-2.5 text-sm {{ $logistics->is_active ? 'text-amber-600' : 'text-emerald-600' }} hover:bg-slate-50 transition-colors">
                                        <i data-lucide="{{ $logistics->is_active ? 'ban' : 'check-circle' }}" class="w-4 h-4 mr-3"></i>
                                        {{ $logistics->is_active ? 'Suspend' : 'Activate' }}
                                    </button>
                                </form>
                                @endcan
                                ...
                                @can('users verify')
                                @if(!$logistics->email_verified_at)
                                <form action="{{ route('admin.logistics.verify-email', $logistics->id) }}" method="POST" class="contents">
                                    @csrf
                                ...
                                <form action="{{ route('admin.logistics.reset-password', $logistics->id) }}" method="POST" class="contents" onsubmit="return confirm('Are you sure you want to reset password?')">
                                    @csrf
                                ...
                                @can('users delete')
                                <form action="{{ route('admin.logistics.destroy', $logistics->id) }}" method="POST" class="contents" onsubmit="return confirm('Delete this user?')">
                                    @csrf @method('DELETE')
