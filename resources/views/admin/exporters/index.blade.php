...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Exporter Management</h1>
        <p class="text-slate-500 text-sm">Manage platform access and permissions</p>
    </div>
    <div class="flex items-center gap-3">
        @can('users logs')
        <a href="{{ route('admin.exporters.all-auth-logs') }}" class="px-4 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-sm flex items-center hover:bg-slate-50 transition-all shadow-sm">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    ...
    <a href="{{ request('view') === 'trash' ? route('admin.exporters.index') : route('admin.exporters.index', ['view' => 'trash']) }}" 
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50"
         x-data="{ 
            ...
            isAllSelected: false,
            toggleAll() {
                if (this.selectedUsers.length === {{ $exporters->count() }}) {
                    this.selectedUsers = [];
                    this.isAllSelected = false;
                } else {
                    this.selectedUsers = [ @foreach($exporters as $exporter) '{{ $exporter->id }}', @endforeach ];
         ...
         x-init="$watch('selectedUsers', value => isAllSelected = value.length === {{ $exporters->count() }})">
    ...
    <div class="p-6 border-b border-slate-50 flex flex-wrap items-center justify-between gap-4">
        ...
        <form action="{{ route('admin.exporters.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
    ...
    <form x-ref="bulkForm" action="{{ route('admin.exporters.bulk-action') }}" method="POST" style="display: none;">
        @csrf
    ...
    <div class="min-h-[450px]">
        <table class="w-full text-left border-separate border-spacing-0">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="group hover:bg-slate-50/50 transition-all" :class="selectedUsers.map(id => Number(id)).includes({{ $exporter->id }}) ? 'bg-slate-50' : ''">
                    <td class="px-8 py-5">
                        <div class="flex items-center">
                            <div class="mr-4">
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" class="hidden" value="{{ $exporter->id }}" x-model="selectedUsers">
                                    <div class="w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center"
                                         :class="selectedUsers.map(id => Number(id)).includes({{ $exporter->id }}) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-200 group-hover:border-emerald-500'">
                                        <i data-lucide="check" class="w-3 h-3 text-white" x-show="selectedUsers.map(id => Number(id)).includes({{ $exporter->id }})"></i>
                                    </div>
                                </label>
                            </div>
                            <div class="w-11 h-11 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden mr-4 group-hover:scale-105 transition-transform">
                                ...
                                @else
                                    <span class="text-xs font-black text-slate-400 uppercase">{{ substr($exporter->name, 0, 2) }}</span>
                                @endif
                            </div>
                            <div>
                                <p class="text-sm font-bold text-slate-800 leading-none mb-1.5">{{ $exporter->name }}</p>
                                <div class="flex items-center gap-2">
                                    <p class="text-xs text-slate-400">{{ $exporter->email }}</p>
                    ...
                    <td class="px-8 py-5 text-center">
                        ...
                        @elseif($exporter->locked_until && $exporter->locked_until->isFuture())
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-50 text-amber-600" title="Locked until {{ $exporter->locked_until->format('M d, H:i') }}">
                                <i data-lucide="lock" class="w-3 h-3 mr-1.5"></i>
                                Locked
                            </span>
                        @elseif($exporter->is_active)
                    ...
                    <td class="px-8 py-5 text-sm font-medium text-slate-500">
                        {{ $exporter->created_at->format('j/n/Y') }}
                    </td>
                    <td class="px-8 py-5 text-right" x-data="{ open: false }">
                        <div class="relative inline-block text-left">
                            ...
                            <div x-show="open" 
                                ...
                                @if(request('view') === 'trash')
                                @can('users delete')
                                <form action="{{ route('admin.exporters.restore', $exporter->id) }}" method="POST" class="contents">
                                    @csrf
                                ...
                                <form action="{{ route('admin.exporters.force-delete', $exporter->id) }}" method="POST" class="contents" onsubmit="return confirm('PERMANENTLY DELETE this account? This cannot be undone!')">
                                    @csrf @method('DELETE')
                                ...
                                @endcan
                                @else
                                <a href="{{ route('admin.exporters.show', $exporter->id) }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
                                    <i data-lucide="eye" class="w-4 h-4 mr-3 text-slate-400"></i>
                                    View Profile
                                </a>
                                @can('users logs')
                                <a href="{{ route('admin.exporters.auth-logs', $exporter->id) }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
                                ...
                                <form action="{{ route('admin.exporters.resend-verification', $exporter->id) }}" method="POST" class="contents">
                                    @csrf
                                ...
                                @can('users reset 2fa')
                                @if($exporter->hasTwoFactorEnabled())
                                <form action="{{ route('admin.exporters.reset-2fa', $exporter->id) }}" method="POST" class="contents" onsubmit="return confirm('Are you sure you want to reset 2FA for this user? They will be forced to set it up again on their next login.')">
                                    @csrf
                                ...
                                @can('users unlock')
                                @if($exporter->locked_until && $exporter->locked_until->isFuture())
                                <form action="{{ route('admin.exporters.unlock', $exporter->id) }}" method="POST" class="contents">
                                    @csrf
                                    <button class="w-full flex items-center px-4 py-2.5 text-sm text-amber-600 hover:bg-amber-50 transition-colors">
                                ...
                                @can('users edit')
                                <a href="{{ route('admin.exporters.edit', $exporter->id) }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
                                ...
                                @can('users email')
                                <button @click="$dispatch('open-email-modal', { id: {{ $exporter->id }}, name: '{{ $exporter->name }}' })" 
                                ...
                                @can('users status')
                                <form action="{{ route('admin.exporters.toggle-status', $exporter->id) }}" method="POST" class="contents">
                                    @csrf
                                    <button class="w-full flex items-center px-4 py-2.5 text-sm {{ $exporter->is_active ? 'text-amber-600' : 'text-emerald-600' }} hover:bg-slate-50 transition-colors">
                                        <i data-lucide="{{ $exporter->is_active ? 'ban' : 'check-circle' }}" class="w-4 h-4 mr-3"></i>
                                        {{ $exporter->is_active ? 'Suspend' : 'Activate' }}
                                    </button>
                                </form>
                                @endcan
                                ...
                                @can('users verify')
                                @if(!$exporter->email_verified_at)
                                <form action="{{ route('admin.exporters.verify-email', $exporter->id) }}" method="POST" class="contents">
                                    @csrf
                                ...
                                <form action="{{ route('admin.exporters.reset-password', $exporter->id) }}" method="POST" class="contents" onsubmit="return confirm('Are you sure you want to reset password?')">
                                    @csrf
                                ...
                                @can('users delete')
                                <form action="{{ route('admin.exporters.destroy', $exporter->id) }}" method="POST" class="contents" onsubmit="return confirm('Delete this user?')">
                                    @csrf @method('DELETE')
